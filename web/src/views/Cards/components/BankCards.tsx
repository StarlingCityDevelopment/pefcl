...
import { Plus, Info, AlertCircle, ShieldEllipsis, ShieldCheck } from 'lucide-react';
...
const IssueCardAction = ({ accountId, onSuccess }: IssueCardActionProps) => {
  ...
  const handleOrderCard = async () => {
    ...
    try {
      ...
      const cardEvent = selectedAccount.type === AccountType.Personal ? CardEvents.OrderPersonal : CardEvents.OrderShared;
  ...
  return (
    <>
      ...
      >
        ...
        <Typography variant="pre" className="text-[10px] font-bold uppercase tracking-widest">{t('Issue credential')}</Typography>
      </motion.button>

      <Modal 
        isOpen={isOpen} 
        onClose={handleClose} 
        title={t('Credential Issuance')}
        maxWidth="xl"
      >
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr,360px] gap-12">
            <div className="flex flex-col gap-8">
              <div className="flex flex-col gap-3">
                ...
                <Typography variant="pre" className="text-[9px] text-slate-600 font-bold uppercase tracking-widest text-center mt-2">
              ...
              <div className="p-8 rounded-[2rem] bg-white/[0.02] border border-white/5 flex flex-col gap-2">
                <Typography variant="pre" className="text-[10px] font-bold text-white/40 uppercase tracking-widest">{t('Origin Entity')}</Typography>
                <Typography className="text-xl font-bold uppercase text-white leading-none">{selectedAccount?.accountName}</Typography>
                <Typography variant="pre" className="text-[10px] font-bold text-slate-600 mt-2">{selectedAccount?.number}</Typography>
              </div>
            </div>
            ...
            <AnimatePresence mode='wait'>
              {error && (
                ...
                >
                  ...
                  <Typography variant="pre" className="text-[10px] font-bold uppercase tracking-widest leading-relaxed">{error}</Typography>
                </motion.div>
              )}
            </AnimatePresence>
...
const BankCards = ({ onSelectCardId, selectedCardId, accountId }: { accountId: number, selectedCardId: number, onSelectCardId(id: number): void }) => {
  const { t } = useTranslation();
  const allCards = useAtomValue(cardsAtom);
  const [, updateCards] = useAtom(cardsAtom);
    ...
    <>
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-1 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 auto-rows-min">
          ...
          <IssueCardAction 
            accountId={accountId} 
            onSuccess={() => updateCards(accountId)} 
          />
          ...
          {cards.length === 0 && (
            <div className="col-span-full py-24 flex flex-col items-center justify-center bg-white/[0.01] border border-dashed border-white/5 rounded-[3.5rem] grayscale opacity-40 gap-4">
              ...
              <Typography variant="pre" className="text-[10px] font-bold text-slate-500 uppercase tracking-widest text-center leading-relaxed">
                {t('No credentials issued for this entity.')}<br/>
