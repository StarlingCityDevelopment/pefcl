...
import { Loader2, ArrowUpCircle, Wallet } from "lucide-react";
...
const Deposit = () => {
  ...
  const isValidTransaction =
    Boolean(amount) && value > 0 && selectedAccountId > 0;

  const { mutate: mutateDeposit, isLoading } = useMutation(
  ...
  const isButtonDisabled =
    !isValidNewBalance || !isValidTransaction || isLoading;
        ...
        <div className="flex flex-col gap-5">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="flex flex-col gap-4">
              <Typography
                variant="label"
                className="text-slate-500 font-black uppercase tracking-widest px-1"
              >
                {t("Destination")}
              </Typography>
              <AccountSelect
            ...
            <div className="flex flex-col gap-1">
              <Typography
                variant="label"
                className="text-slate-500 font-black uppercase tracking-widest px-1 mb-3"
              >
                {t("Amount")}
              </Typography>
              <PriceField
              ...
              {!isValidNewBalance && value > 0 && (
                <Typography
                  variant="pre"
                  className="text-red-500 text-[10px] uppercase tracking-widest mt-1 pl-2"
                >
              ...
              <div className="mt-2 pl-1">
                <NewBalance
                  ...
                  newBalanceText={t("Post-Deposit Wallet")}
                />
              </div>
            </div>
          </div>
