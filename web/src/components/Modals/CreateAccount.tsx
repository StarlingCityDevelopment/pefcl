...
const CreateAccountModal: React.FC<{ onClose(): void }> = ({ onClose }) => {
 ...
 return (
  ...
  control={control}
  rules={{
  required: { value: true, message: t('Account name is required') },
  maxLength: { value: 25, message: t('Account name is too long') },
  pattern: { value: regexAlphaNumeric, message: t('Invalid account name') },
  }}
  ...
  />
  <Typography variant="pre" className="text-sm font-medium text-slate-400 group-hover:text-white transition-colors">
